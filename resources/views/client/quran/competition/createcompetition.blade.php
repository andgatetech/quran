...
@include('client.layouts.top-bar')
    ...
    <div class="container1">
        <div class="tabs">
        <button class="tab-btn active " onclick="window.location.href='{{ route('quran.competition.create') }}'">Create
            Competition</button>
          <button class="tab-btn " onclick="window.location.href='{{ route('quran.competition.list') }}'">Competition
            List</button>
        </div>
    </div>
    <div class="container">
        ...
        <div class="form-container">
          <form class="competition-form" method="POST" action="{{ route('quran.competition.store') }}">
            @csrf
            @php

      @endphp
            <input type="text" name="main_name" placeholder="Competition Main Name" required>
            <input type="hidden" name="sub_name" placeholder="Competition Sub Name" required

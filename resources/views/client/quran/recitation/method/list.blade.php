...
@include('client.layouts.top-bar')
    ...
    <div class="tabs">
    <button class="tab-btn" onclick="window.location.href='{{ route('quran.recitation.method.create') }}'">Create Recitation Method</button>
    <button class="tab-btn active" onclick="window.location.href='{{ route('quran.recitation.method.list') }}'">Recitation Method List</button>
    </div>
    ...
    <div class="container">
      ...
      <div class="list-container">
        ...
        @foreach($recitationMethods as $recitationMethod)
          <div class="category-card">
            <div class="card-header" onclick="toggleDropdown(this)">
              <p>Recitation Method: <span>{{ $recitationMethod->name }}</span> <i class="fas fa-chevron-down"></i></p>
            </div>
            <div class="card-actions">
              <form action="{{ route('quran.recitation.method.delete', $recitationMethod->id) }}" method="POST" style="display:inline-block;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn delete-btn">Delete</button>
              </form>
                <a href="{{ route('quran.recitation.method.edit',$recitationMethod->id) }}" class="btn edit-btn">Edit</a>
            </div>
          </div>
        @endforeach

        @if($recitationMethods->isEmpty())
          <p>No recitation method found. Click "Create Recitation Method" to add one.</p>
        @endif
      </div>
      </div>

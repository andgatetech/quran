...
<header class="header">
    ...
    <h1>Competition List(Quiz)</h1>
  </header>

    <div class="container1">
        <div class="tabs">
        <button class="tab-btn" onclick="window.location.href='{{ route('quiz.competition.create') }}'">Create Competition</button>
        <button class="tab-btn active" onclick="window.location.href='{{ route('quiz.competition.list') }}'">Competition List</button>
        </div>
    </div>
    <div class="container">
        <div class="row">
        ...
        @foreach($competitions as $competition)
          <div class="competition-card">
            ...
            <div class="card-buttons">
              <form action="{{ route('quiz.competition.delete', $competition->id) }}" method="POST" style="display:inline-block;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn delete-btn">Delete</button>
              </form>
                <a href="{{ route('quiz.competition.edit',$competition->id) }}" class="btn edit-btn">Edit</a>
            </div>
          </div>
        @endforeach

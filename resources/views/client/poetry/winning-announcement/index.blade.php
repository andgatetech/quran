...
<body>
    ...
    <script>
        $(document).ready(function () {
            const fetchWinners = () => {
                $.ajax({
                    url: '{{ route('poetry.winning.fetch-winners') }}', // Fetch data from this route
                    method: 'GET',
                        ...
                        competitors.forEach(competitor => {
                            ...
                            const competitorHtml = `
                                ...
                                <div class="action-buttons">
                                    <form action="{{ route('poetry.competitor.announce', '') }}/${competitor.id}" method="POST" style="display:inline-block; cursor:pointer">
                                        @csrf
                                        <button class="action-button announce-button" type="submit">Announce</button>
                                    </form>

                                    <form action="{{ route('poetry.competitor.recheck', '') }}/${competitor.id}" method="POST" style="display:inline-block; cursor:pointer">
                                        @csrf

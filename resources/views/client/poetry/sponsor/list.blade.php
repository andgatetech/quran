...
@section('content')
    ...
    <header class="header">
        ...
        <h1> Sponsor List(Poetry)</h1>
    </header>

    <div class="tabs">

        <button class="tab-btn " onclick="window.location.href='{{ route('poetry.sponsors.create') }}'">Create Sponsor</button>
        <button class="tab-btn active" onclick="window.location.href='{{ route('poetry.sponsors.index') }}'">Sponsor
            List
        </button>
    </div>
                    ...
                    <div class="list-item mb-3 p-3 border rounded" onclick="toggleDetails(event, this)">
                        ...
                        <div class="details mt-2" style="display: none;">
                            ...
                            <div class="button-group-inline mt-3">
                                <form action="{{ route('poetry.sponsors.destroy', $sponsor->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a href="{{ route('poetry.sponsors.edit', $sponsor->id) }}" class="btn btn-edit btn-primary">Edit</a>
                                <a href="{{ route('poetry.sponsors.show', $sponsor->id) }}" class="btn btn-view btn-info">View Photo</a>
                            </div>
                        </div>
                    </div>
                @endif

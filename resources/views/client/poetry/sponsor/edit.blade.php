...
<button class="tab-btn "  onclick="window.location.href='{{ route('poetry.sponsors.create') }}'">Create Sponsor</button>
<button class="tab-btn " onclick="window.location.href='{{ route('poetry.sponsors.index') }}'">Sponsor List</button>
</div>
  </div>
    ...
    <div class="container my-5">
        ...
        <!-- The Form -->
        <form action="{{ route('poetry.sponsors.update', $sponsor->id) }}" method="POST" enctype="multipart/form-data" class="form-container mt-4">
            @csrf
            @method('PUT')
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control" id="status" name="status" required>
                    <option value="Enable" {{ old('status', $sponsor->status) == 'Enable' ? 'selected' : '' }}>Enable</option>
                    <option value="Disable" {{ old('status', $sponsor->status) == 'Disable' ? 'selected' : '' }}>Disable</option>
                </select>
            </div>

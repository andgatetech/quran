...
@section('content')
<header class="header">
    <a class="back-btn" href="{{ route('poetry.managenertificate.create') }}"><i class="fas fa-home"></i></a>
    <h1>Certificate List(Poetry)</h1>
</header>
...
<div class="tabs">
    ...
    <div class="nav-buttons">
        <button class="nav-btn" onclick="window.location.href='{{ route('poetry.managenertificate.create') }}'">Certificate Settings</button>
        <button class="nav-btn active" onclick="window.location.href='{{ route('poetry.managenertificate.index') }}'">Settings List</button>
        <button class="nav-btn" onclick="window.location.href='{{ route('poetry.managenertificate.generate.view') }}'">Generate</button>
        <button class="nav-btn" onclick="window.location.href='{{ route('poetry.managenertificate.generated.list') }}'">Generated List</button>
    </div>
...
<div id="certificate-search" class="container my-3">
        <form style="padding: 10px;" action="{{ route('poetry.managenertificate.index') }}" method="GET" enctype="multipart/form-data" class="d-block">
        @csrf
            ...
            <div class="list-item mb-3 p-3 border rounded" onclick="this.classList.toggle('active')">
                ...
                <div class="details mt-2" style="display: none;">
                    ...
                    <!-- Edit and Delete Buttons -->
                        <a href="{{ route('poetry.managenertificate.edit', $certificate->id) }}" class="btn btn-edit btn-warning">Edit</a>
                        <form action="{{ route('poetry.managenertificate.destroy', $certificate->id) }}" method="POST" style="display:inline-block;">
                            @csrf
                            @method('DELETE')
...
<form action="{{ route('poetry.certificate.generate') }}" method="POST" target="_blank">
    @csrf

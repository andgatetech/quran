...
class QuestionController extends Controller
{
    ...
    public function checkCurriculum(Request $request)
    {
        ...
        $ageId = $request->input('age_category_id');
        $sideId = $request->input('side_category_id');
        $readId = $request->input('read_category_id');

        $curriculum = Curriculum::where('competition_id', $competitionId)
            ->where('age_category_id', $ageId)
            ->where('side_category_id', $sideId)
            ->where('read_category_id', $readId)
            ->first();

        return response()->json([
            'exists' => $curriculum ? true : false,
    ...
    public function getAyat($bookNumber)
    {
        $ayat = Quran::where('juz_number', $bookNumber)->get(['id', 'ayah_text']);
    ...
    public function list(Request $request)
    {
        ...
        if ($request->has('sideCategory') && $request->sideCategory != '') {
            $questions->where('side_category_id', $request->sideCategory);
        }
        if ($request->has('readCategory') && $request->readCategory != '') {
            $questions->where('read_category_id', $request->readCategory);
        }
        if ($request->has('ageCategory') && $request->ageCategory != '') {
            $questions->where('age_category_id', $request->ageCategory);
        }

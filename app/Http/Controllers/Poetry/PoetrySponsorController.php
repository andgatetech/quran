...
class PoetrySponsorController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'Tin' => 'nullable|string|max:255', // Validate Tin number
            'Details' => 'nullable|string', // Validate Details (optional)
            'status' => 'required|in:Enable,Disable', // Ensure status is either Enable or Disable
        ]);
        ...
        try {
            ...
            return redirect()->route('poetry.sponsors.create')->with('success', 'Sponsor created successfully.');
        } catch (\Exception $e) {
            return redirect()->route('poetry.sponsors.create')->with('error', 'Failed to create sponsor: ' . $e->getMessage());
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validatedData = $request->validate([
            ...
            'Tin' => 'nullable|string', // Tin field validation
            'Details' => 'nullable|string', // Details field validation
            'status' => 'required|in:Enable,Disable', // Status field validation
        ]);
        ...
        try {
            ...
            $sponsorData = [
                'name' => $validatedData['name'],
                'competition_id' => $validatedData['competition_id'],
                'Tin' => $validatedData['Tin'] ?? null,  // Set Tin field if provided
                'Details' => $validatedData['Details'] ?? null,  // Set Details field if provided
                'status' => $validatedData['status'],  // Set Status field
            ];
            ...
            return redirect()->route('poetry.sponsors.index')->with('success', 'Sponsor updated successfully!');
        } catch (\Exception $e) {
            // Log the error
    ...
    public function edit($id)
    {
        ...
        return view('client.poetry.sponsor.edit', compact('sponsor','competitions'));
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            // Redirect to sponsor list with success message
            return redirect()->route('poetry.sponsors.index')->with('success', 'Sponsor deleted successfully!');
        } catch (\Exception $e) {
            \Log::error('Error deleting sponsor: ' . $e->getMessage());
            return redirect()->route('poetry.sponsors.index')->with('error', 'Failed to delete sponsor. Please try again.');
        }
    }
    ...
    public function show($id)
    {
        ...
        return view('client.poetry.sponsor.view', compact('sponsor','competitions'));
    }
}

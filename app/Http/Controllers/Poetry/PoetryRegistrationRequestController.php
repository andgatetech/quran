...
class PoetryRegistrationRequestController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        ->when($competition_id, function($query) use($competition_id){
            $query->where('competition_applications.competition_id',$competition_id);
        })
        ->when($age_category, function($query) use($age_category){
            $query->where('competition_applications.age_category',$age_category);
        })
        ->when($side_category, function($query) use($side_category){
            $query->where('competition_applications.side_category',$side_category);
        })
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($request->status === 'Approved') {
            ...
            try {
                ...
                return redirect()->route('poetry.competition.applicant.list', ['status' => $request->status])
